...
pub fn find_keyboard_device() -> Result<Device> {
    for i in 0..20 {
        let path = format!("/dev/input/event{}", i);
        if let Ok(device) = Device::open(&path) {
            if device.name().is_some()
                && device.supported_keys().map_or(false, |keys| keys.contains(Key::KEY_FN))
            {
...
pub fn start_keyboard_handler(tx: Sender<InputEvent>) -> Result<()> {
                ...
                for ev in events {
                    if let InputEventKind::Key(key) = ev.kind() {
                        if key.code() == Key::KEY_FN.code() {
                            ...
                            tx.send(InputEvent::Keyboard(key)).unwrap();
                        }
                    }
                }
            }
...
pub fn find_touch_device() -> Result<Device> {
    for i in 0..20 {
        let path = format!("/dev/input/event{}", i);
        if let Ok(device) = Device::open(&path) {
            if device.name() == Some("Apple Inc. Apple T1 Controller Touchpad") {
