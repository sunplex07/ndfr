...
impl Button {
    pub fn draw(&self, c: &Context, height: f64) -> Result<()> {
        let color = if self.active { BUTTON_COLOR_ACTIVE } else { BUTTON_COLOR_INACTIVE };
...
pub enum Page {
    ...
    ControlStripExpanding(Vec<Button>),
    ControlStripClosing(Vec<Button>),
}

pub fn create_default_layout(width: i32, _height: i32) -> Result<Vec<Button>> {
    ...
    for button_config in layout.left.buttons {
        let content = if let Some(icon_name) = button_config.icon {
            ...
            ButtonContent::Text(button_config.text.unwrap_or_default())
        };
    ...
    let right_buttons_width: f64 = layout.right.buttons.iter().map(|b| b.width).sum::<f64>()
        + layout.right.spacing * (layout.right.buttons.len() - 1) as f64;
    current_x = width as f64 - right_buttons_width;

    let num_right_buttons = layout.right.buttons.len();
    for (i, button_config) in layout.right.buttons.into_iter().enumerate() {
        let content = if let Some(icon_name) = button_config.icon {
            let icon_path = find_resource_path(&format!("icons/{}", icon_name))?;
            let svg_data = std::fs::read(icon_path)?;
            let tree = Tree::from_data(&svg_data, &usvg::Options::default())?;
...
pub fn create_expanded_layout(width: i32, _height: i32) -> Result<Vec<Button>> {
    ...
    for (icon_name, action, rounded_corners, is_group_ender) in button_definitions {
        ...
        buttons.push(Button {
            content: ButtonContent::Icon(tree),
            action,
            active: false,
            x: current_x,
            width: button_width,
