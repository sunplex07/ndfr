...
pub fn draw_ui(surface: &ImageSurface, page: &Page, animation_progress: f64, is_screenshot: bool) -> Result<()> {
    ...
    match page {
        Page::Default(buttons) | Page::FnKeys(buttons) => {
            for button in buttons {
                button.draw(&c, height)?;
            }
        }
        ...
        Page::ControlStripExpanding(buttons) => {
            ...
            for button in buttons {
                ...
                new_button.draw(&c, height)?;
            }
        }
        Page::ControlStripClosing(buttons) => {
            let total_width: f64 = buttons.iter().map(|b| b.width).sum::<f64>() + (buttons.len() as f64 - 1.0) * 2.0;
            let start_x = width - total_width;
            for button in buttons {
                let mut new_button = button.clone();
